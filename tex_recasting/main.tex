...
while new ideas % on how 
to probe new models and non-trivial signatures and to improve the sensitivity of existing searches constantly emerge.
...
like detector smearing (Delphes~\cite{deFavereau:2013fsa}) results and simple object smearing. \textcolor{blue}{[SK: ????]}

Further improvements in the presentation of the results and their recastability
will include correlations among the signal systematics, \textcolor{blue}{[SK: this already exists,; the CMS SUSY group is providing correlation matrices, they are simply not used yet by the recast codes]}
as well as the possibility to provide unfolded distributions for key observables. \textcolor{blue}{[SK: really?]}
Finally, in the future it might become possible to directly use particle-level measurements to constrain new models. \textcolor{blue}{[SK: this is also being done already; see Contur, or the contribution on Higgs fiducial cross sections in these proceedings]}
...
(Analysis Description Format, LHADA Proposal) \textcolor{blue}{[SK: IMHO this should be changed, there are no results based on LHADA in this contribution]} and frameworks like
CheckMate~\cite{Drees:2013wra,Cacciari:2005hq},
MadAnalysis~\cite{Conte:2012fm,Conte:2014zja,Dumont:2014tja}
...
\textcolor{blue}{[SK: need to say which signature was simulated for the comparison and how!]} 
The pseudo-data samples have been generated by using MadGraph5\_aMC@NLO~\cite{Alwall:2014hca} 
and Pythia8~\cite{Sjostrand:2014zea} \textcolor{blue}{\it .... is not enough information.}
...
\textcolor{blue}{[SK: again, need to say which signature was simulated for the comparison and how!]}
...
and Rivet (\textcolor{blue}{\it give reference of analysis code}). Whilst a fair agreement is
...
\textcolor{blue}{\bf [SK: to my mind if the two sections below don't have results, they should be taken out. 
It can be said in the conclusions that the comparison is ongoing with several more analyses which are currently being validated]}
...
in the recasting frameworks and the validation is ongoing.
...
Implementations of this analysis in the recasting frameworks is ongoing. 


\section*{CONCLUSIONS}
We presented a first benchmark comparison of the performance of different recasting tools 
which reproduce ATLAS and CMS analyses in Monte Carlo simulation.
