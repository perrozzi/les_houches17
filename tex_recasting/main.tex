...
The idea behind the exercise described in this section is the implementation of KHC analyses of increasing complexity, both in the language-independent format (Analysis Description Format, LHADA Proposal) and frameworks like CheckMate, MadAnalysis, Rivet, followed by a comparison of the results.
...
Once the analysis are available, we try to reproduce the New Physics interpretation of the original paper, validating their implementation.
A further step consists in the recasting of the analysis other New Physics model and compare the results among the different frameworks.

Aside the current scope of the exercise, it would be interesting to check how the performance from Delphes are with general-purpose and not analysis-specific cards, since this is usually the preferred use.
